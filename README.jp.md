...
![alt text](assets/core_comp.png)  
   1. MCP Client (mcp_client.py)
      ...
      - Provides tool name mapping and standardization functionality
...
![alt text](assets/image_process1.png)
   ...
   3. If the model needs to use tools, the MCP client calls the appropriate MCP server
   4. Tool call results are returned to the model, which generates the final response
...
> Tips: If you need to configure multiple account ak/sk using a polling mechanism, you can add a `credential.csv` in the conf/ directory with column names **ak** and **sk**, and fill in multiple ak/sk, for example:
...
- In the chatbot interface, add this JSON file. Note: This [browser use](https://github.com/vinayak-mehta/mcp-browser-use) server starts the browser in headed mode by default, making it suitable for demos deployed on local computers. If deploying on a server, please add the phrase `use headless is true to initialize the browser` in your prompt.
```json
{ "mcpServers": 
	{ "mcp-browser": 
		{ "command": "uvx", 
...
Task input: `Help me prepare a comprehensive introduction about Xiaomi SU7 Ultra, including performance, price, special features, with rich text and images, and save it as a beautiful HTML file in the local directory. If referencing images from other websites, ensure the images exist and are accessible.`  
